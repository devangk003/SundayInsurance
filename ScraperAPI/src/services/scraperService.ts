...
import { QuoteRequest, ScraperResult, InsuranceQuote } from '../types';
import logger from '../utils/logger';
import config from '../config/config';
import readline from 'readline';

// Helper function to extract text from elements
async function scrapeOptions(page: puppeteer.Page, selector: string): Promise<string[]> {
  logger.info(`Scraping options with selector: ${selector}`);
  ...
  logger.info(`Scraped ${results.length} options: ${results.slice(0, 5).join(', ')}${results.length > 5 ? '...' : ''}`);
  return results;
}

// New helper function to search for items in large lists
async function searchAndSelectOption(page: puppeteer.Page, searchTerm: string, searchBoxSelector: string, optionsSelector: string): Promise<boolean> {
  logger.info(`Searching for '${searchTerm}' in dropdown`);
  ...
  const matchingOptions = await page.evaluate((selector, term) => {
    ...
    if (matches.length > 0) {
      (matches[0].element as HTMLElement).click();
  ...
  if (matchingOptions.success) {
    logger.info(`Successfully selected: ${matchingOptions.selected}`);
    return true;
  } else {
    logger.warn(`No exact matches found for '${searchTerm}'. Available matches: ${JSON.stringify(matchingOptions.matches)}`);
    return false;
  }
}
...
export async function scrapeInsuranceQuotes(request: QuoteRequest): Promise<ScraperResult> {
  ...
  try {
    logger.info(`Starting scraping session. New car: ${request.isNewCar ? 'Yes' : 'No'}`);
    ...
    if (request.isNewCar) {
      // ===== NEW CAR FLOW =====
      logger.info('Starting new car flow');
      ...
      try {
        logger.info('Looking for New Car button');
        ...
        logger.info('Clicked on New Car option');
        ...
        logger.error(`Failed to click New Car button: ${(error as Error).message}`);
        ...
        for (const selector of alternativeSelectors) {
          try {
            ...
            clicked = true;
            logger.info(`Successfully clicked using selector: ${selector}`);
            break;
          } catch (err) {
            logger.warn(`Failed with selector ${selector}: ${(err as Error).message}`);
          }
        }
      ...
      logger.info('Looking for See All Brands button');
      ...
      try {
        ...
        for (const selector of brandButtonSelectors) {
          try {
            logger.info(`Trying brand button selector: ${selector}`);
            await page.waitForSelector(selector, { timeout: 5000 });
            await page.click(selector);
            brandButtonFound = true;
            logger.info(`Clicked on See All Brands using selector: ${selector}`);
            break;
          } catch (err) {
            logger.warn(`Failed with brand button selector ${selector}`);
          }
        }
        
        if (!brandButtonFound) {
          // Try to inspect what's on the page
          logger.info('Could not find See All Brands button. Inspecting page content...');
          ...
          if (manufacturersExist) {
            logger.info('Already on manufacturers page, proceeding');
          } else {
            throw new Error('Could not find See All Brands button with any selector');
          }
        }
      } catch (error) {
        logger.error(`Failed to find or click See All Brands: ${(error as Error).message}`);
        throw error;
      }
      
      // Wait for the manufacturers list to load
      try {
        logger.info('Waiting for manufacturers list to load');
        await page.waitForSelector('div.all-make > div.all-make__item', { timeout: 10000 });
      } catch (error) {
        logger.error(`Failed to find manufacturers list: ${(error as Error).message}`);
      ...
      if (manufacturerSelector) {
        await (manufacturerSelector as ElementHandle<Element>).click();
        logger.info(`Clicked on manufacturer: ${selectedMake}`);
      } else {
        throw new Error(`Could not find manufacturer: ${selectedMake}`);
      }
      
      // Step 2: Get and present car models
      logger.info('Looking for See All Cars button');
      ...
      try {
        ...
        if (seeAllCarsButton) {
          ...
          if (element) {
            ...
            logger.info('Clicked on "See All Cars" button');
            modelsButtonClicked = true;
            
            // Wait for the models to load after clicking
            await new Promise(resolve => setTimeout(resolve, 2000));
          } else {
            logger.warn('Could not find "See All Cars" button - handle did not resolve to an element');
          }
        } else {
          logger.warn('Could not find "See All Cars" button using element search');
        }
      } catch (error) {
        logger.error(`Error finding/clicking "See All Cars" button: ${(error as Error).message}`);
      }
      ...
      if (!modelsButtonClicked) {
        ...
        logger.info('Checking for top-models container');
        const topModelsExists = await page.evaluate(() => {
          return document.querySelector('.top-models') !== null;
        });
        
        if (topModelsExists) {
          logger.info('Found top-models container - using this for model selection');
          ...
          logger.info(`Found ${topModels.length} models in top-models container: ${topModels.join(', ')}`);
          
          // Present models to user and get selection
          const selectedModel = await promptUser('Select car model', topModels);
          logger.info(`User selected model: ${selectedModel}`);
          ...
          const clicked = await page.evaluate((modelName) => {
            ...
            for (const span of spans) {
              if (span.textContent?.trim() === modelName) {
                (span.closest('.top-models__item') as HTMLElement).click();
                return true;
              }
            }
            return false;
          ...
          if (clicked) {
            logger.info(`Clicked on model: ${selectedModel} from top-models container`);
            modelsButtonClicked = true; // Set this to skip other selectors
          } else {
            logger.error(`Failed to click on model: ${selectedModel} from top-models container`);
          }
        }
        ...
        if (!modelsButtonClicked) {
          ...
          for (const selector of possibleSelectors) {
            try {
              ...
              if (exists) {
                await page.click(selector);
                logger.info(`Clicked on models button using selector: ${selector}`);
                modelsButtonClicked = true;
              ...
              logger.warn(`Failed with selector ${selector}: ${(error as Error).message}`);
            }
          }
        }
      }
      ...
      if (modelElements === 0) {
        logger.warn(`No models found with selector: ${modelSelector}. Trying alternatives...`);
        ...
        for (const selector of alternativeSelectors) {
          ...
          if (count > 0) {
            logger.info(`Found ${count} models with alternative selector: ${selector}`);
      ...
      logger.info(`Found ${models.length} car models: ${models.join(', ')}`);

      // If we still don't have models, search more broadly for anything that might be a model
      if (models.length === 0) {
        logger.warn('No models found with any specific selectors. Looking for any clickable elements...');
        ...
        logger.info(`Found ${possibleModels.length} possible model elements on page`);
        ...
        if (possibleModels.length > 0) {
          ...
          logger.info(`Using ${filteredModels.length} possible models: ${filteredModels.join(', ')}`);
          ...
          logger.info(`Clicked on model by text search: ${selectedModel}`);
        } else {
        ...
        // Find and click the selected model
        const modelSelectorHandle = await page.evaluateHandle((model: string, selector: string) => {
          const elements = document.querySelectorAll(selector);
          for (const el of elements) {
        ...
        if (modelSelectorHandle) {
          ...
          if (element) {
            // Cast the element handle to ElementHandle<Element> before clicking
            await (element as ElementHandle<Element>).click();
            logger.info(`Clicked on model: ${selectedModel}`);
          } else {
            ...
            if (clicked) {
              logger.info(`Clicked on model by text content: ${selectedModel}`);
            } else {
      ...
      if (fuelTypeSelector) {
        await (fuelTypeSelector as ElementHandle<Element>).click();
        logger.info(`Clicked on fuel type: ${selectedFuelType}`);
      } else {
      ...
      logger.info('Waiting for variant selection to load');
      ...
      try {
        ...
        logger.info('Clicked on variant dropdown');
        ...
        if (variants.length > 0) {
          ...
          logger.info(`Selected variant: ${selectedVariant}`);
          variantSelectionWorked = true;
        }
      } catch (error) {
        logger.warn(`Dropdown variant selection failed: ${(error as Error).message}`);
      ...
      if (!variantSelectionWorked) {
        logger.info('Trying alternative variants-list structure');
        ...
        try {
          ...
          if (variantsListExists) {
            logger.info('Found variants-list structure');
            ...
            logger.info(`Found ${variants.length} variants in variants-list: ${variants.join(', ')}`);
            ...
            if (variants.length > 0) {
              ...
              const clicked = await page.evaluate((variantText) => {
                ...
                for (const span of spans) {
                  if (span.textContent?.trim() === variantText) {
                    (span.closest('.variants-list__item') as HTMLElement).click();
                    return true;
                  }
                }
                return false;
              ...
              if (clicked) {
                logger.info(`Clicked variant: ${selectedVariant} from variants-list`);
                variantSelectionWorked = true;
              } else {
                logger.error(`Failed to click variant: ${selectedVariant} in variants-list`);
              }
            }
          } else {
            logger.warn('Alternative variants-list structure not found');
          }
        } catch (error) {
          logger.error(`Alternative variant selection failed: ${(error as Error).message}`);
        }
      }

      // If both methods failed, throw an error
      if (!variantSelectionWorked) {
        logger.error('Failed to select a variant using either method');
      ...
      logger.info(`Found ${regPlaces.length} registration places`);

      // Ask user for a search term
      const regSearchTerm = await promptUser('Enter your city name or RTO code (e.g. "Mumbai" or "MH-01")');
      logger.info(`User is searching for: ${regSearchTerm}`);

      // Filter the list based on user's search term
      const filteredRegPlaces = regPlaces.filter(place => 
        place.toLowerCase().includes(regSearchTerm.toLowerCase())
      );
      ...
      logger.info(`User selected registration place: ${selectedRegPlace}`);
      ...
      logger.info('Checking for registration year selection after registration place...');
      ...
      if (hasYearDropdown) {
        ...
        if (hasSearchBox) {
          ...
          if (yearClicked) {
            logger.info(`Clicked on year ${selectedYear} using search`);
          } else {
          ...
          logger.info(`Selected year ${selectedYear} directly`);
        }
        ...
        if (hasSaveButton) {
          logger.info('Registration year not needed, but Save & Continue button found');
          await page.click('button.w--button--fb-registration');
          logger.info('Clicked Save & Continue button');
      ...
      try {
        ...
        logger.info('Claim question found for new car flow');
        ...
        if (request.hasMadeClaim) {
          logger.info('Setting claim status as Yes for new car');
          ...
          try {
            ...
            const yesClicked = await page.evaluate(() => {
              ...
              const yesOption = options.find(opt => 
                opt.textContent?.toLowerCase().includes('yes')
              );
              
              if (yesOption) {
                (yesOption as HTMLElement).click();
                return true;
              }
              return false;
            });
            ...
            logger.info('Successfully clicked Yes for claim in new car flow');
          } catch (error) {
            logger.error(`Failed to click Yes for claim in new car flow: ${(error as Error).message}`);
            await page.screenshot({ path: 'claim-yes-failure-newcar.png' });
          }
        } else {
          logger.info('Setting claim status as No for new car');
          ...
          try {
            ...
            logger.info('Successfully clicked No for claim in new car flow');
          } catch (error) {
            logger.error(`Failed to click No for claim in new car flow: ${(error as Error).message}`);
        ...
        logger.warn(`Claim selection section not found in new car flow: ${(error as Error).message}`);
        logger.info('Continuing without claim selection as it might be optional');
        await page.screenshot({ path: 'claim-section-not-found-newcar.png' });
      }

      // Wait for the phone number field to appear
      await page.waitForSelector('input[type="tel"]', { timeout: 10000 });
      logger.info('Phone number field found');

      // After registration year selection for new cars, handle the policy expiry section
      if (request.isNewCar) {
        try {
          logger.info('Checking for policy expiry section after registration year...');
          ...
          logger.info(`Filled in phone number: ${request.phoneNumber}`);
          ...
          for (const selector of viewQuotesButtonSelectors) {
            try {
              ...
              if (exists) {
                await page.waitForSelector(selector, { timeout: 5000 });
                await page.click(selector);
                logger.info(`Clicked "View Quotes" button using selector: ${selector}`);
                quotesButtonClicked = true;
                break;
              }
            } catch (error) {
              logger.warn(`View Quotes button selector ${selector} failed: ${(error as Error).message}`);
            }
          }
          ...
          if (!quotesButtonClicked) {
            ...
            if (quotesButtonClicked) {
              logger.info('Clicked "View Quotes" button using text search');
            } else {
              logger.warn('Could not find "View Quotes" button with any selector');
            }
          }
          ...
          logger.error(`Error handling policy expiry section: ${(error as Error).message}`);
          logger.info('Continuing anyway as this section might be optional in some flows');
      ...
      logger.info('Waiting for quotes to load after flow completion...');
    } else {
      // ===== EXISTING CAR FLOW =====
      logger.info(`Starting existing car flow for registration: ${request.registrationNumber}`);
      ...
      if (request.registrationNumber) {
        ...
        logger.info(`Entered registration number: ${request.registrationNumber}`);
      } else {
        logger.warn('No registration number provided for existing car flow');
      }
      ...
      logger.info(`Filled phone number: ${request.phoneNumber}`);
      ...
      logger.info('Clicked view quotes button');
      ...
      try {
        ...
        logger.info('Confirmation screen loaded successfully');
        ...
        if (request.isPolicyExpired) {
          logger.info('Setting policy as Expired');
          ...
          try {
            ...
            const expiredClicked = await page.evaluate(() => {
              ...
              const expiredOption = options.find(opt => 
                opt.textContent?.toLowerCase().includes('expired') && 
                !opt.textContent?.toLowerCase().includes('not expired')
              );
              
              if (expiredOption) {
                (expiredOption as HTMLElement).click();
                return true;
              }
              return false;
            });
            ...
            logger.info('Successfully clicked Expired option');
          } catch (error) {
            logger.error(`Failed to click Expired option: ${(error as Error).message}`);
            // Take screenshot of the failure
            await page.screenshot({ path: 'expired-click-failure.png' });
          }
        } else {
          logger.info('Setting policy as Not Expired');
          // Try multiple approaches to click the "Not Expired" option
          try {
            // First try: Use text content if available
            const notExpiredClicked = await page.evaluate(() => {
              const options = Array.from(document.querySelectorAll('.w--radio--fl-expiry .w--radio__options .w--radio__option'));
              const notExpiredOption = options.find(opt => 
                opt.textContent?.toLowerCase().includes('not expired')
              );
              
              if (notExpiredOption) {
                (notExpiredOption as HTMLElement).click();
                return true;
              }
              return false;
            });
            ...
            logger.info('Successfully clicked Not Expired option');
          } catch (error) {
            logger.error(`Failed to click Not Expired option: ${(error as Error).message}`);
            // Take screenshot of the failure
        ...
        await page.waitForSelector('.claim-mopro .w--radio--claim', { timeout: 10000 });
        logger.info('Claim section found');
        ...
        if (request.hasMadeClaim) {
          logger.info('Setting claim status as Yes');
          // Try multiple approaches to click the "Yes" option
          try {
            // First try: Use text content
            const yesClicked = await page.evaluate(() => {
            ...
            logger.info('Successfully clicked Yes for claim');
          } catch (error) {
            logger.error(`Failed to click Yes for claim: ${(error as Error).message}`);
            await page.screenshot({ path: 'claim-yes-failure.png' });
          }
        } else {
          logger.info('Setting claim status as No');
          // Try multiple approaches to click the "No" option
          try {
            // First try: Use text content
            const noClicked = await page.evaluate(() => {
            ...
            logger.info('Successfully clicked No for claim');
          } catch (error) {
            logger.error(`Failed to click No for claim: ${(error as Error).message}`);
        ...
        logger.warn(`Confirmation screen not found: ${(error as Error).message}`);
        logger.info('Continuing with quotes page directly as confirmation screen may be skipped sometimes');
      }
    }
    ...
    try {
      ...
      logger.info('Quotes container found, extracting data...');
      ...
      logger.info(`Basic data extraction complete. Found ${basicQuotes.length} quotes.`);
      
      // Process detailed information
      const enhancedQuotes: InsuranceQuote[] = [];
      
      // Process each quote to get detailed information
      for (let i = 0; i < basicQuotes.length; i++) {
        const quote = basicQuotes[i];
        logger.info(`Processing details for quote ${i+1} (${quote.insurer})`);
        ...
        try {
          ...
          for (const selector of planDetailSelectors) {
            try {
              ...
              if (exists) {
                await page.waitForSelector(selector, { timeout: 5000 });
                await page.click(selector);
                logger.info(`Clicked plan details using selector: ${selector}`);
                detailsClicked = true;
                break;
              }
            } catch (error) {
              logger.warn(`Selector ${selector} failed: ${(error as Error).message}`);
            }
          }
          ...
          if (!detailsClicked) {
            ...
            if (detailsClicked) {
              logger.info(`Clicked plan details using text search for quote ${i+1}`);
            } else {
          ...
          logger.info(`Details popup opened for quote ${i+1}`);
          ...
          const detailedInfo = await page.evaluate(() => {
            // Define proper types for our structures 
            type PremiumBreakupItem = { key: string; value: string };
            type PremiumBreakupSection = { name: string; items: PremiumBreakupItem[] };
            type CoverageItem = { title: string; description: string };
            
            const additionalDetails = {
              leftPanel: {
                idvValue: null as string | null,
                ncbPercentage: null as string | null,
                planType: null as string | null, 
                finalPrice: null as string | null
              },
              premiumBreakup: {
                sections: [] as PremiumBreakupSection[],
                total: null as string | null,
                gst: null as string | null,
                finalAmount: null as string | null
              },
              planDetails: {
                covered: [] as CoverageItem[],
                notCovered: [] as string[]
              }
            };
            ...
            try {
              ...
              if (premiumBreakupTabActive) {
                ...
                sectionHeaders.forEach(header => {
                  ...
                  if (!sectionName || sectionName === 'Taxes') return;
                  
                  const section: PremiumBreakupSection = { name: sectionName, items: [] };
          ...
          if (detailedInfo.premiumBreakup.sections.length > 0 && detailedInfo.planDetails.covered.length === 0) {
            logger.info(`Switching to Plan Details tab for quote ${i+1}`);
            ...
            for (const selector of tabSelectors) {
              try {
                ...
                if (exists) {
                  await page.waitForSelector(selector, { timeout: 5000 });
                  await page.click(selector);
                  logger.info(`Switched to Plan Details tab using selector: ${selector}`);
                  tabSwitched = true;
                  break;
                }
              } catch (error) {
                logger.warn(`Tab selector ${selector} failed: ${(error as Error).message}`);
              }
            }
            ...
            if (!tabSwitched) {
              ...
              if (tabSwitched) {
                logger.info(`Switched to Plan Details tab using text search`);
              } else {
                logger.warn(`Failed to switch to Plan Details tab for quote ${i+1}`);
              }
            }
          ...
          if (detailedInfo.planDetails.covered.length > 0 && detailedInfo.premiumBreakup.sections.length === 0) {
            logger.info(`Switching to Premium Breakup tab for quote ${i+1}`);
            ...
            for (const selector of tabSelectors) {
              try {
                ...
                if (exists) {
                  await page.waitForSelector(selector, { timeout: 5000 });
                  await page.click(selector);
                  logger.info(`Switched to Premium Breakup tab using selector: ${selector}`);
                  tabSwitched = true;
                  break;
                }
              } catch (error) {
            ...
            if (!tabSwitched) {
              ...
              if (tabSwitched) {
                logger.info(`Switched to Premium Breakup tab using text search`);
              } else {
                logger.warn(`Failed to switch to Premium Breakup tab for quote ${i+1}`);
              }
            }
          ...
          logger.info(`Details captured for quote ${i+1}: ${JSON.stringify(detailedInfo, null, 2)}`);
          ...
          for (const selector of closeSelectors) {
            try {
              ...
              if (exists) {
                await page.waitForSelector(selector, { timeout: 5000 });
                await page.click(selector);
                logger.info(`Closed details popup using selector: ${selector}`);
                closeClicked = true;
                break;
              }
            } catch (error) {
              logger.warn(`Close button selector ${selector} failed: ${(error as Error).message}`);
            }
          }
          ...
          if (!closeClicked) {
            ...
            if (closeClicked) {
              logger.info(`Closed details popup using text search`);
            } else {
              logger.warn(`Could not find close button for quote ${i+1}, trying to continue anyway`);
          ...
          logger.error(`Error processing details for quote ${i+1}: ${(error as Error).message}`);
      ...
      logger.info(`Enhanced data extraction complete for all quotes.`);
      ...
      logger.error(`Error extracting quotes: ${(error as Error).message}`);
      return {
        success: false,
        quotes: [],
    ...
    logger.error(`Scraping error: ${(error as Error).message}`);
...
export { scrapeOptions, searchAndSelectOption, sanitizeText };